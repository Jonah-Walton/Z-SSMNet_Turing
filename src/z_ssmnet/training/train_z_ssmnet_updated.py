...
def main(taskname="Task2302_z-nnmnet"):
    ...
    # Check nnUNet CLI exists
    if shutil.which("nnunet") is None:
        raise RuntimeError("ERROR: 'nnunet' command not found. "
    ...
    for fold in args.folds:
        ...
        pretrained_weights = pretrainedweights_dir / "ssl_mnet_zonal.model"

        cmd = [
            "nnunet", "plan_train", str(taskname),
            str(workdir),
            "--results", str(checkpoints_dir),
            "--trainer", "myTrainer_zonal",
            "--fold", str(fold),
            "--custom_split", str(split_file),
        ...
        try:
            ...
            print(f"Training failed on fold {fold}: {e}")
            return
    ...
    for checkpoint in ["model_best", "model_final_checkpoint"]:
        for fold in args.folds:
            ...
            for ext in [".model", ".model.pkl"]:
                src = fold_dir / f"{checkpoint}{ext}"
                dst = out_fold / f"{checkpoint}{ext}"
